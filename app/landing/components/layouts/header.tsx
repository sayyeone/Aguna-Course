  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="container mx-auto flex justify-between items-center py-5 px-4 lg:px-0">
        ...
        <nav className="hidden lg:flex gap-16 font-medium text-dark">
        ...
        {menuOpen && (
          <div className="absolute top-full left-0 right-0 bg-white shadow-lg z-50 flex flex-col gap-5 px-6 py-8 lg:hidden border-t border-gray-100">
            <Link href="/" className="font-medium hover:text-primary transition-colors text-lg" onClick={() => setMenuOpen(false)}>Home</Link>
            <Link href="#category-section" className="font-medium hover:text-primary transition-colors text-lg" onClick={() => setMenuOpen(false)}>Category</Link>
            <Link href="#products-section" className="font-medium hover:text-primary transition-colors text-lg" onClick={() => setMenuOpen(false)}>Explore Products</Link>
          </div>
        )}
      </div>
      {isCartPopupOpen && <div className="lg:hidden relative px-4 pb-4 bg-white"><CartPopup close={() => setIsCartPopupOpen(false)} /></div>}
    </header>
  );
};
